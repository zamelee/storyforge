...
#### 25.5.1 历史年表按世界切换（补完多世界）

- `historicalTimelineEvents` / `historicalKeywords` 增加可选 `worldGroupId`；`History` 单例已具备
- HistoryPanel 顶部接入 `WorldGroupSwitcher`，概述 + 时间线事件 + 关键词整体跟随世界切换
- 新建事件/关键词时盖章当前世界；单世界模式 `worldGroupId=null` 走原逻辑
- **AI 上下文关系**：历史年表本就是世界观内容，已通过 `buildCurrentWorldContext` 注入写作链路；本项只是让其按世界隔离，不改注入逻辑

#### 25.5.2 故事进程年表（AI 从已写正文提取，不是写作前规划）

> **定位**：与"历史年表（世界背景）"严格区分——这是**正文里发生过的剧情大事**，是写作的**下游产物**，方向是「已写小说 → AI 提取 → 年表」，而非「年表 → 小说」。

- **复用已有能力**：当前每章写完后 AI 已从正文提取 5 类状态卡（含 `category='event'`），`EventTimeline` 已按章节展示。本功能是把它升级为独立的、可整本提取的「故事年表」
- 新增独立入口（创作区或著作信息下），与"故事线（结构）""历史年表（背景）"区分
- 一键「从已写章节提取故事年表」：对整本/选定章节范围跑提取（复用 `state-extract-adapter` 的事件提取逻辑）
- AI 把零散事件梳理为：故事内纪年（如"开元三年春"）+ 重要度分级 + 因果关联
- 用户可在 AI 结果上手动增删改
- **AI 上下文关系**：这是**纯产物**，默认独立于写作链路（不自动注入，避免污染）。可选增强：允许用户把年表作为"前情提要"片段注入后续章节写作（作为 `buildContext` 的可选数据源，开关控制），但默认关闭，不动主流程
...
**优先级建议**：25.5.1（补完多世界，小）> 25.5.4（可视化，中）> 25.5.3（灵感反推增强，中）> 25.5.2（故事年表，中大，依赖事件提取打磨）
